...
- `demo.js` (128 líneas) — 10 tiempos hardcodeados, `setInterval` cada 1.5 s revela los largos, al terminar muestra la card de IA y llama a `analyze({mode:'summary'})`. Botón pasa a "Reiniciar demo". **Mejora post-plan:** selector de 2 videos (`static/video/demo1.mp4`, `demo2.mp4`) que se reproducen de fondo durante la demo; cambiar de video resetea la demo.
