...
Recibe `multipart/form-data` con el campo `video`, crea una sesión de tracking remota y responde `text/event-stream`. Cada evento contiene `{time,width,height,boxes,identity_summary?,count,lap_scores?,tracking_diagnostics?,lap_decisions?}`. Cuando está disponible, el Monitor muestra `identity_summary.confirmed_count` como personas físicas; `identity_id` conserva esa persona aunque ByteTrack cambie `track_id`. `count` mantiene su semántica legacy de claves de caja acumuladas. `lap_scores` conserva el score heurístico y la evidencia por carril cuando la calibración está habilitada; `tracking_diagnostics` aparece sólo si se habilita explícitamente. En modo shadow, `lap_decisions` aparece una sola vez cuando un episodio cruza el threshold configurado: el Monitor los muestra como largos detectados sin mezclar dos identidades del mismo carril. El archivo temporal y la sesión remota se limpian al terminar, fallar o cortar el stream. Consulta [INTEGRACION_IA.md](INTEGRACION_IA.md) para el contrato completo.
