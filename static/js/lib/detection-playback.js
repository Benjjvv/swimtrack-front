 ...
 * @param {{id:number,identity_id?:number,swimmer_id?:number,x1:number,y1:number,x2:number,y2:number,conf:number}} box
...
function toDetection(box, scale, offX, offY) {
  return {
    ...
    // El ordinal visible se asigna al confirmar una identidad.  No mostramos
    // IDs canónicos tentativos ni los tracklets efímeros de ByteTrack.
    swimmerId: box.swimmer_id,
    bbox: [
      offX + box.x1 * scale,
      offY + box.y1 * scale,
      (box.x2 - box.x1) * scale,
